// components/properties/PropertyConsultants.tsx
// Consultores imobiliários explicitamente associados a este imóvel para
// partilha (tabela property_consultants) — distinto de NearbyConsultants.tsx,
// que é apenas uma sugestão automática por zona.
'use client'
import { useState } from 'react'
import Link from 'next/link'
import type { Person } from '@/types'

type Consultant = { id: string; person_id: string; people: { id: string; name: string; phone: string | null; email: string | null } }

export function PropertyConsultants({ propertyId, consultants, onChange }: {
  propertyId: string
  consultants: Consultant[]
  onChange: () => void
}) {
  const [picking, setPicking] = useState(false)
  const [search, setSearch] = useState('')
  const [results, setResults] = useState<Person[]>([])
  const [loading, setLoading] = useState(false)

  async function doSearch(q: string) {
    setSearch(q)
    if (q.trim().length < 2) { setResults([]); return }
    setLoading(true)
    try {
      const res = await fetch(`/api/people?search=${encodeURIComponent(q.trim())}&types=consultor`)
      if (res.ok) setResults(await res.json())
    } finally { setLoading(false) }
  }

  async function associate(personId: string) {
    const res = await fetch(`/api/properties/${propertyId}/assigned-consultants`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ person_id: personId }),
    })
    if (res.ok) { setPicking(false); setSearch(''); setResults([]); onChange() }
  }

  async function remove(personId: string) {
    const res = await fetch(`/api/properties/${propertyId}/assigned-consultants?person_id=${personId}`, { method: 'DELETE' })
    if (res.ok) onChange()
  }

  const labelStyle = { fontSize: 9, letterSpacing: '0.2em', textTransform: 'uppercase' as const, color: 'var(--muted)', marginBottom: 4, fontWeight: 500 }
  const inputStyle = { background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 7, padding: '8px 12px', fontSize: 12, color: 'var(--text)', outline: 'none', fontFamily: 'Jost, sans-serif', width: '100%' }

  return (
    <div style={{ background: 'var(--card)', border: '1px solid var(--border)', borderRadius: 12, padding: 24 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14 }}>
        <div className="font-display" style={{ fontSize: 15 }}>Partilha / consultores</div>
        {!picking && (
          <button onClick={() => setPicking(true)} style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 8, padding: '4px 12px', fontSize: 11, color: 'var(--text)', cursor: 'pointer', fontFamily: 'Jost, sans-serif' }}>
            + Adicionar consultor
          </button>
        )}
      </div>

      {!picking && (
        consultants.length > 0 ? (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {consultants.map(c => (
              <div key={c.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 8, padding: '8px 12px' }}>
                <div>
                  <Link href={`/people/${c.people.id}`} style={{ fontSize: 13, fontWeight: 600, color: 'var(--text)', textDecoration: 'none' }}>{c.people.name}</Link>
                  <div style={{ fontSize: 11, color: 'var(--muted)' }}>{c.people.phone || c.people.email || ''}</div>
                </div>
                <button onClick={() => remove(c.person_id)} style={{ background: 'none', border: 'none', color: '#EF4444', fontSize: 11, cursor: 'pointer' }}>Remover</button>
              </div>
            ))}
          </div>
        ) : (
          <div style={{ fontSize: 12, color: 'var(--muted)' }}>Nenhum consultor associado.</div>
        )
      )}

      {picking && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          <div style={labelStyle}>Procurar consultor</div>
          <input style={inputStyle} autoFocus placeholder="Nome, telefone ou email..." value={search} onChange={e => doSearch(e.target.value)} />
          <div style={{ display: 'flex', flexDirection: 'column', gap: 4, maxHeight: 220, overflowY: 'auto' }}>
            {loading && <div style={{ fontSize: 12, color: 'var(--muted)' }}>A procurar...</div>}
            {!loading && search.trim().length >= 2 && results.length === 0 && <div style={{ fontSize: 12, color: 'var(--muted)' }}>Nenhum consultor encontrado.</div>}
            {results.map(p => (
              <button key={p.id} onClick={() => associate(p.id)} style={{ textAlign: 'left', background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 8, padding: '8px 12px', cursor: 'pointer', fontFamily: 'Jost, sans-serif' }}>
                <div style={{ fontSize: 13, fontWeight: 600, color: 'var(--text)' }}>{p.name}</div>
                <div style={{ fontSize: 11, color: 'var(--muted)' }}>{p.phone || p.email || ''}</div>
              </button>
            ))}
          </div>
          <button onClick={() => { setPicking(false); setSearch(''); setResults([]) }} className="btn btn-ghost" style={{ fontSize: 12, alignSelf: 'flex-start' }}>Cancelar</button>
        </div>
      )}
    </div>
  )
}
