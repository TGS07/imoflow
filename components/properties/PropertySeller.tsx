// components/properties/PropertySeller.tsx
'use client'
...
export function PropertySeller({ propertyId, seller, onChange }: {
  propertyId: string
  seller: Seller | null
  onChange: () => void
}) {
  ...
  async function setSeller(sellerId: string | null) {
  ...
  return (
    <div style={{ background: 'var(--card)', border: '1px solid var(--border)', borderRadius: 12, padding: 24 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14 }}>
        <div className="font-display" style={{ fontSize: 15 }}>Vendedor</div>
        {!picking && (
      ...
      {!picking && (
        seller ? (
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            <button onClick={() => setSeller(null)} style={{ background: 'none', border: 'none', color: '#EF4444', fontSize: 11, cursor: 'pointer' }}>Remover</button>
          </div>
        ) : (
          <div style={{ fontSize: 12, color: 'var(--muted)' }}>Nenhum vendedor associado.</div>
        )
      )}
      ...
      {picking && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 4, maxHeight: 220, overflowY: 'auto' }}>
            ...
            {results.map(p => (
              <button key={p.id} onClick={() => setSeller(p.id)} style={{ textAlign: 'left', background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 8, padding: '8px 12px', cursor: 'pointer', fontFamily: 'Jost, sans-serif' }}>
