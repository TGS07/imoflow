// components/contacts/ConsultantProperties.tsx
// Imóveis onde este contacto (consultor imobiliário) está associado para
// partilha — via tabela property_consultants. Reaproveita o mesmo endpoint
// de associação usado na ficha do imóvel (PropertyConsultants.tsx).
'use client'
import { useState } from 'react'
import Link from 'next/link'

type PropertyRef = { id: string; title: string; status: string; price: number | null; reference: string | null }
type Association = { id: string; property: PropertyRef }

export function ConsultantProperties({ personId, associations, onChange }: {
  personId: string
      ...
      {!picking && (
        associations.length > 0 ? (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {associations.map(a => (
              <div key={a.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 8, padding: '8px 12px' }}>
